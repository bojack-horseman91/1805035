...
void Object::getDiffuseAndSpecularColor(points3D normal, points3D intersectionPoint, points3D reflectedRay)
{
    ...
    for (LightSource *lightSource : myLightSources)
    {
        ...
        double cur_lambert = std::max(toSource.dot(normal) * reflectionCoefficient.diffuse + epsilon, 0.0);
        lambert += cur_lambert;

        double cur_phong = pow(std::max(reflectedRay.dot(toSource), 0.0), shininess) * scalingFactor;
        phong += cur_phong;
    }
